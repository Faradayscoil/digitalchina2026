...
def load_artifact(model_name, farm_id):
    ...
    if not os.path.exists(artifact_path):
        raise FileNotFoundError(
            f'Missing preprocess artifact for {model_name} farm {farm_id}: {artifact_path}')
...
def load_trained_model(model_name, farm_id, artifact):
    ...
    if not os.path.exists(best_weights_path):
        raise FileNotFoundError(
            f'Missing model or weights for {model_name} farm {farm_id}: '
...
def load_actual_power_series(data_path, index, capacity=None):
    raw_df = pd.read_csv(data_path, parse_dates=['时间'])
    if '时间' not in raw_df.columns or TARGET_COL not in raw_df.columns:
        return pd.Series(np.nan, index=index, dtype=float)

    raw_df = raw_df[['时间', TARGET_COL]].sort_values('时间').drop_duplicates('时间')
...
def prepare_prediction_arrays(test_file, artifact):
    ...
    # The target column is allowed only as already-observed history in each input window.
    # Future target values stay outside model inputs and are used only for metrics/plots.
    ...
    if missing_cols:
        for col in missing_cols:
            df[col] = 0.0
        print(f'Warning: missing test columns filled with 0: {missing_cols}')
...
def make_prediction_dataset(features, history_len=HISTORY_LEN, forecast_len=FORECAST_LEN):
    ...
    if n_samples <= 0:
        raise ValueError('Test file is too short for one full history/forecast window.')
...
def save_single_window_plot(pred_df, model_name, farm_id, dirs, forecast_len=FORECAST_LEN):
    ...
    try:
        ...
        print(f'{model_name} farm {farm_id} single-window plot failed: {exc}')
        figure_path = None
...
def save_weighted_full_test_plot(pred_df, model_name, farm_id, dirs, capacity=None):
    ...
    try:
        ...
        print(f'{model_name} farm {farm_id} weighted plot failed: {exc}')
        figure_path = None
...
def predict_one_farm(model_name, test_file):
    ...
    print(f'\n===== Predict {model_name} / farm {farm_id} =====')
    ...
    if y_pred.shape[0] != n_samples:
        raise ValueError(
            f'{model_name} farm {farm_id} prediction count mismatch: '
            f'{y_pred.shape[0]} vs {n_samples}')
    ...
    print(f"{model_name} farm {farm_id}: MAE={all_metrics['mae']:.4f}, "
...
def predict_model_family(model_name, test_files):
    ...
    for test_file in test_files:
        try:
            ...
            print(f'Skip {model_name} {os.path.basename(test_file)}: {exc}')
            continue
    ...
    print(f'{model_name} summary saved: {summary_path}')
    print(f'{model_name} horizon metrics saved: {all_horizon_path}')
...
if __name__ == '__main__':
    ...
    if not test_files:
        raise FileNotFoundError(f'No {TEST_FILE_PATTERN} files found in {DATA_DIR}')

    requested_model_names = get_requested_model_names()
    invalid = sorted(set(requested_model_names) - set(DEFAULT_MODEL_NAMES))
    if invalid:
        raise ValueError(f'Unknown model names: {invalid}')

    print(f'Found {len(test_files)} wind test files.')
    print(f'Predict models: {requested_model_names}')
    ...
    if all_summary:
        ...
        global_summary_path = os.path.join(
            BASE_RESULT_DIR,
            'wind_dl_other_models_test_metrics_summary.csv',
        )
        global_summary.to_csv(global_summary_path, index=False, encoding='utf-8-sig')
        print(f'All-model summary saved: {global_summary_path}')

    if all_horizon:
        global_horizon = pd.concat(all_horizon, ignore_index=True)
        global_horizon_path = os.path.join(
            BASE_RESULT_DIR,
            'wind_dl_other_models_test_metrics_by_horizon_all.csv',
        )
        global_horizon.to_csv(global_horizon_path, index=False, encoding='utf-8-sig')
        print(f'All-model horizon metrics saved: {global_horizon_path}')

    print('Other deep-learning model prediction finished.')
